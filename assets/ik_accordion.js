;(function ( $, window, document, undefined ) {
	...
	.on('keydown', {'plugin': plugin}, plugin.onKeyDown) // enable keyboard navigation
		...
		this.panels = $elem.children('dd').each(function(i, el) {
			...
			$me.attr({
				'id': id,
				'role': 'region', // add role region to reach panel
	...
	Plugin.prototype.togglePanel = function (event) {
		...
		if(plugin.options.autoCollapse) { // expand current panel and collapse the rest
			...
			$btn = $me;
			var panelId = $btn.attr('aria-controls');
                        var ddElement = $elem.find('dd#' + panelId);
	
			isVisible = !!$panel.is(':visible');
			if (isVisible) {
				$btn.attr('aria-hidden', 'true');
				$btn.attr('aria-expanded', 'false');
				ddElement.attr('aria-hidden', 'true');
				$btn.removeClass('expanded');	
			}
			else {
				$btn.attr('aria-hidden', 'false');
				$btn.attr('aria-expanded', 'true');
				ddElement.attr('aria-hidden', 'false');
				$btn.addClass('expanded');
			}
